...
RUN --mount=type=cache,target=/var/cache/dnf,z dnf -y --enablerepo=updates-testing install golang rpm-ostree selinux-policy-targeted jq dnf-utils rsync wget
# Copy in the source
COPY . /src
ARG KIND=bootc
ARG VARIANT=full
...
RUN --mount=type=cache,target=/var/cache/dnf,z --mount=type=cache,target=/workdir rm -rf /workdir/rootfs && /src/build/install-imagedir /src/${KIND}-${VARIANT} /workdir/rootfs
